...
mainpath = os.getcwd()
enhancer_dir = Path(mainpath + '/enhancer/')
sys.path.append(str(enhancer_dir))
...
if __name__ == '__main__':
    ...
    dataset_dir = './data/prepare_final'
    head_pose_name = './data/target/head.npy'
    head_ckpt_dir = './checkpoints/supervise'
    save_dir = dataset_dir + '/enhanced/'
    weight_name = './data/target/weights.npy'
    index_name = './data/target/indices.npy'
    ...
    os.system('rm -rf ./data/prepare_final/original')
    os.system('rm -rf ./data/prepare_final/ref')
    os.system('cp -a ./data/part/test_sync ./data/prepare_final/original')
    os.system('cp -a ./data/part/ref ./data/prepare_final/ref')

    image_folder = dataset.ImageFolderDataset(dataset_dir, is_test=True, supervise=True)
    head_dataset = dataset.FaceCropDataset(image_folder, head_pose_name, image_transforms, crop_size=crop_size,
                                           weight_name=weight_name, index_name=index_name)
    ...
    os.system('rm -rf ./data/target/train/train_fg')
    os.system('cp -a ./data/prepare_final/enhanced ./data/target/train/train_fg')
    os.system('rm -rf ./data/prepare_final')

    os.system('rm -rf ./data/target/train/train_bg')
    os.system('cp -a ./data/target/train/fill_bg ./data/target/train/train_bg')
