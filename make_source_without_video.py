...
syn_bg_dir = save_dir.joinpath('syn_bg')
syn_bg_dir.mkdir(exist_ok=True)
...
for idx in tqdm(range(len(os.listdir(str(json_dir))))):
    ...
    cv2.imwrite(str(syn_bg_dir.joinpath('{:05}.png'.format(idx))), bg)
    img = bg
    ...
    try:
        mask = cv2.imread('./data/target/train/train_mask/00000.png')
