...
os.system('cp -a ../data/target/train/train_fg ../data/part/ref/')
...
for data in tqdm(dataset):
    ...
    visuals = OrderedDict([('synthesized_image', util.tensor2im(generated.data[0])),
                           ('input_image', util.tensor2im((data['image'][:, :, 0, :, :][0]) * (data['mask'][0])))])
    img_path = data['path']
...
synthesized_image_dir = './prepare/fg/target/test_latest/images/'
for img_idx in tqdm(range(len(os.listdir(synthesized_image_dir)) // 2)):
    ...
    img = cv2.imread(synthesized_image_dir+'{:05}_input_image.jpg'.format(img_idx))
    cv2.imwrite(str(test_real_dir) + '/{:05}.png'.format(img_idx), img)
