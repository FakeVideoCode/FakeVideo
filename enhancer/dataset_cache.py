...
class ImageFolderDataset(Dataset):
    def __init__(self, root, cache=None, is_test=False, supervise=False):
...
class FaceCropDataset(Dataset): #TODO FaceCropDataset
    ...
    def __getitem__(self, item):
        ...
        if self.weight_name != '':
            ...
            _, fake_head0, _, _, _, _, _, _ = self.get_full_sample(indices[0])
            _, fake_head1, _, _, _, _, _, _ = self.get_full_sample(indices[1])
            _, fake_head2, _, _, _, _, _, _ = self.get_full_sample(indices[2])
            fake_head = (weights[0] * fake_head0 + weights[1] * fake_head1 + weights[2] * fake_head2) / 2 + 0.5 * syn_head
