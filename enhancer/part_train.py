...
def main(is_debug, part):
    ...
    pose_name = '../data/target/head.npy'.replace('head', part)
    ckpt_dir = '../checkpoints/head'.replace('head', part)
    log_dir = '../checkpoints/head/logs'.replace('head', part)
    batch_num = 10
    batch_size = 10

    gen_name = os.path.join(ckpt_dir, '%05d_generator.pth' % 40000)
    dis_name = os.path.join(ckpt_dir, '%05d_discriminator.pth' % 40000)
    if os.path.isfile(gen_name) and os.path.isfile(dis_name):
        print(part, 'done.')
        return
...
if __name__ == '__main__':
    cudnn.enabled = True
    is_debug = False
    part_list = ['head', 'lhand', 'rhand', 'lfoot', 'rfoot']
