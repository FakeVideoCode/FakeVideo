import model
import dataset
# from enhancer.trainer import Trainer
...
def main(is_debug, part):
    # configs
    import os

    dataset_dir = '../data/part'
    pose_name = '../data/target/head.npy'
    ckpt_dir = '../checkpoints/supervise'
    log_dir = '../checkpoints/supervise/logs'
    weight_name = '../data/target/weights.npy'
    index_name = '../data/target/indices.npy'
    batch_num = 1000
    batch_size = 10

    image_folder = dataset.ImageFolderDataset(dataset_dir, supervise=True)
    face_dataset = dataset.FaceCropDataset(image_folder, pose_name, image_transforms, crop_size=48, weight_name=weight_name, index_name=index_name)
    data_loader = DataLoader(face_dataset, batch_size=batch_size, drop_last=True, num_workers=0, shuffle=True)
