...
# todo discriminator-temporal
class Discriminator_t(nn.Module):
    def __init__(self, n_class=1000, chn=8, debug=False, target='fg'):
        super().__init__()
        ...
        # if target == 'fg':
        #     pre0 = [nn.Conv3d(3, 3, kernel_size=(2, 1, 1), padding=0)]
        # elif target == 'bg':
        #     pre0 = [nn.Conv3d(3, 3, kernel_size=(3, 1, 1), padding=0)]
        # pre1 = [nn.BatchNorm2d(3), nn.ReLU()]
        # self.pre0 = nn.Sequential(*pre0)
        # self.pre1 = nn.Sequential(*pre1)

        if target == 'fg':
            self.pre_conv = nn.Sequential(SpectralNorm(nn.Conv2d(6, 1 * chn, 3, padding=1), ),
                                          nn.ReLU(),
                                          SpectralNorm(nn.Conv2d(1 * chn, 1 * chn, 3, padding=1), ),
                                          nn.AvgPool2d(2))
            self.pre_skip = SpectralNorm(nn.Conv2d(6, 1 * chn, 1))
        elif target == 'bg':
            self.pre_conv = nn.Sequential(SpectralNorm(nn.Conv2d(9, 1 * chn, 3, padding=1), ),
                                          nn.ReLU(),
                                          SpectralNorm(nn.Conv2d(1 * chn, 1 * chn, 3, padding=1), ),
                                          nn.AvgPool2d(2))
            self.pre_skip = SpectralNorm(nn.Conv2d(9, 1 * chn, 1))
    ...
    def forward(self, input, class_id):
        ...
        # size = input.size()
        # input = self.pre0(input)
        # input = input.view(size[0], size[1], size[3], size[4])
        ...
        return F.sigmoid(F.sigmoid(out_linear + prod))
